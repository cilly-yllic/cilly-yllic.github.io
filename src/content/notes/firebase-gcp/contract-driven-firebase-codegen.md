...
フロントエンドとバックエンドで型を共有したい、という問題は昔からあります。ひと昔前は両者のリポジトリを分けるのが一般的だったので、同じ型定義をそれぞれに手書きする — つまり重複させるしかありませんでした。
...
ところが、Cloud SQL (Data Connect) を SoT、Firestore を読み取り投影面 (キャッシュ) とする構成（[Firestore を Public Cache として扱う](/notes/firebase-gcp/firestore-as-public-cache)）を採ると、型共有の軸が変わります。「1 つの型を両側から参照する」では済まず、**同じモデルがレイヤーごとに別の表現で何度も書かれる** ようになるのです。モノレポは「同じ型をどこからでも参照できる」ようにはしてくれますが、「別の表現に分かれた定義を一致させ続ける」ことまでは面倒を見てくれません。
