...
description: 'The design of a CI pipeline that keeps DNS, auth, delivery certificates, and env values — everything left over after config-driven GCP / Firebase provisioning — following the same source of truth.'
category: 'firebase-gcp'
publishedAt: 2026-06-30
---

In [Provisioning GCP / Firebase environments from a single settings.yml](/en/notes/firebase-gcp/config-driven-gcp-firebase-provisioning/), I covered getting GCP projects and the Firebase platform generated automatically from a settings file. But that's not the end. Add one environment and a chain of chores appears outside the foundation:
...
![Architecture diagram of the Platform Sync pipeline](/images/platform-sync-pipeline-architecture-en.svg)
...
![Flow of waiting for the certificate to go ACTIVE before enabling the proxy](/images/cloudflare-cert-gated-proxy-en.svg)
