...
description: 'Structurally eliminating the drift of maintaining the same model across Data Connect, Firestore, Zod, and API DTOs, with code generation from a YAML contract as the single source of truth.'
...
But once you adopt an architecture where Cloud SQL (Data Connect) is the source of truth and Firestore is a read projection (cache) — see [Treating Firestore as a public cache](/notes/firebase-gcp/firestore-as-public-cache) (Japanese) — the axis of type sharing changes. It's no longer "one type referenced from both sides"; **the same model gets written over and over in different representations, one per layer**. A monorepo lets you reference the same type from anywhere, but it does nothing to keep definitions that have split into different representations consistent with each other.
...
![Architecture diagram: contract.yml as the single source of truth, compiled through an IR into per-layer code](/images/contract-driven-firebase-codegen-architecture.png)
